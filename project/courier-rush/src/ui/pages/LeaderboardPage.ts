import type { GameMode, LeaderboardRecord } from "../../types.js";
...
export function LeaderboardPage(): HTMLElement {
  ...
  let seedRecords: LeaderboardRecord[] = [];

  const header = createElement("div", "retro-window");
  header.append(
    createElement("h1", "hero-title", "Leaderboard"),
    createElement("span", "hero-subtitle", "РЕЙТИНГ ЗА РЕЖИМАМИ")
  );
  ...
  function renderRows(records: LeaderboardRecord[]): void {
    updateStats(records);
    const body = table.querySelector<HTMLTableSectionElement>("tbody");
    if (!body) {
      return;
    ...
    records.forEach((record, index) => {
      ...
      row.append(
        ...
        createElement("td", "", record.playerName),
        createElement("td", "", String(record.score)),
        createElement("td", "", String(record.delivered)),
        createElement("td", "", record.date)
      );
      body.appendChild(row);
    });
  }

  function updateStats(records: LeaderboardRecord[]): void {
...
function createStatBox(label: string, value: string): { box: HTMLElement; value: HTMLElement } {
  const box = createElement("div", "stat-box");
  box.append(
    createElement("span", "stat-label", label),
    createElement("strong", "stat-value", value)
  );
  const valueNode = box.querySelector("strong") as HTMLElement;
