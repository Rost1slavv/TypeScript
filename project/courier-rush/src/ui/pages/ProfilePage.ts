...
export function ProfilePage(): HTMLElement {
  ...
  header.append(
    createElement("h1", "hero-title", "Profile"),
    createElement("span", "hero-subtitle", "LOCAL PLAYER CARD"),
    createElement(
      "p",
      "hero-text",
      "Профіль гравця зберігається у localStorage. Тут можна змінити ім’я, переглянути кількість зіграних змін, успішні доставки, найкращий результат і середній рахунок."
    )
  );
  ...
  const input = createElement("input", "profile-input") as HTMLInputElement;
  input.type = "text";
  input.maxLength = 32;
...
function createStat(container: HTMLElement, label: string, value: string): HTMLElement {
