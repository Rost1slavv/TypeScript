import type { Direction, GameMode, GameSnapshot, Order, ResultSummary } from "../../types.js";
...
export function GamePage(): { element: HTMLElement; destroy: () => void } {
  ...
  const modeOverlayText = createElement(
    "p",
    "mode-overlay-text",
    "Класика — ручний вибір замовлень. Аркада — 90 секунд випадкових доставок на очки."
  );
  ...
  const applySelectedMode = (mode: GameMode): void => {
    selectedMode = mode;
  ...
  const togglePause = (): void => engine?.togglePause();
  ...
  const actionKeyHandler = (event: KeyboardEvent): void => {
  ...
  function createStats(container: HTMLElement): StatNodes {
  ...
  function createStatBox(container: HTMLElement, label: string, value: string): HTMLElement {
  ...
  function createOrdersRenderKey(snapshot: GameSnapshot): string {
  ...
  function renderOrders(snapshot: GameSnapshot): void {
  ...
  function createOrderCard(order: Order, snapshot: GameSnapshot): HTMLElement {
    ...
    meta.append(
      createElement("span", `badge ${order.priority}`, `пріоритет: ${priorityLabel(order.priority)}`),
      createElement("span", "badge", `вага: ${order.weightKg} кг`),
      createElement("span", "badge", `₴${order.reward}`),
      createElement("span", "badge", `дедлайн: ${formatTime(timeLeft)}`)
    );
  ...
  function handleFinish(result: ResultSummary): void {
    if (resultSaved) {
      return;
    }
    resultSaved = true;
...
function createModeSelector(onSelect: (mode: GameMode) => void, compact = false): { root: HTMLElement; setActive: (mode: GameMode, locked: boolean) => void } {
  ...
  return {
    root,
    setActive: (mode: GameMode, locked: boolean) => {
...
function createMobilePad(onChange: (direction: Direction, pressed: boolean) => void, onBoost: (pressed: boolean) => void): HTMLElement {
