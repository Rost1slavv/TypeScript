...
export class GameEngine {
  ...
  private readonly classicDurationSeconds = 240;
  private readonly arcadeDurationSeconds = 90;
  ...
  private readonly worldHeight = 1260;
  private readonly roads: Road[] = this.createRoads();
  private readonly crosswalks: Crosswalk[] = this.createCrosswalks();
  private readonly buildings: Building[] = this.createBuildings();
  private readonly parks: ParkArea[] = this.createParks();
  private readonly speedCameras: SpeedCamera[] = this.createSpeedCameras();
  private readonly initialOrders: Order[];
  private readonly depot: Coordinate = { x: 92, y: 1070 };
  private readonly depotBuilding: DepotBuilding = { x: 22, y: 1160, width: 132, height: 62 };
  private readonly keys: KeyMap = { up: false, down: false, left: false, right: false };
  private readonly redPenaltyTimes = new Map<string, number>();
  private readonly speedCameraPenaltyTimes = new Map<string, number>();
  private readonly speedConfig: SpeedConfig = {
    normalMax: 190,
    boostMax: 315,
    acceleration: 520,
    braking: 620,
  ...
  private currentVelocity: Coordinate = { x: 0, y: 0 };
  private courierDirection: Direction = "right";
  private selectedOrderId: string | null = null;
  private currentMode: GameMode = "classic";
  private message = "Обери режим і натисни «Старт».";
  private elapsedSeconds = 0;
  private timeLeft = this.settings.durationSeconds;
  ...
  private camera: Coordinate = { x: 0, y: 500 };
  private trafficLights: TrafficLight[] = this.createTrafficLights();
  private trafficCars: TrafficCar[] = this.createTrafficCars();
  private pedestrians: Pedestrian[] = this.createPedestrians();
  ...
  constructor(canvas: HTMLCanvasElement, orders: Order[], options: GameEngineOptions = {}) {
  ...
  addFinishListener(listener: FinishListener): void {
  ...
  setMobileDirection(direction: Direction, pressed: boolean): void {
    if (!this.isInteractive) {
      return;
    }
  ...
  setBoosting(pressed: boolean): void {
    if (!this.isInteractive) {
      return;
    }
  ...
  private updateCourier(deltaSeconds: number): void {
    const vector = this.getManualVector();
    const hasInput = vector !== null;
    let targetVelocity: Coordinate = { x: 0, y: 0 };
  ...
  private getManualVector(): Coordinate | null {
  ...
  private checkRedLightPenalty(direction: Direction): void {
  ...
  private checkSpeedCameraPenalty(): void {
  ...
  private triggerCameraFlash(camera: SpeedCamera, penalty: number, currentSpeed: number): void {
  ...
  private updateTrafficLights(deltaSeconds: number): void {
  ...
  private updateTrafficCars(deltaSeconds: number): void {
  ...
  private shouldTrafficCarStop(car: TrafficCar, direction: Direction): boolean {
  ...
  private mustKeepDistanceFromCarAhead(car: TrafficCar, direction: Direction): boolean {
    const axis = direction === "left" || direction === "right" ? "horizontal" : "vertical";
  ...
  private mustYieldAtUncontrolledCrossing(_car: TrafficCar, _direction: Direction): boolean {
    return false;
  }

  private pointAhead(position: Coordinate, direction: Direction, distance: number): Coordinate {
    if (direction === "right") {
  ...
  private isPointControlledByTrafficLight(point: Coordinate): boolean {
    return this.trafficLights.some((light) => Math.abs(point.x - light.x) <= 34 && Math.abs(point.y - light.y) <= 34);
  }

  private mustStopAtRedLight(position: Coordinate, direction: Direction): boolean {
    const axis = direction === "left" || direction === "right" ? "horizontal" : "vertical";
    for (const light of this.trafficLights) {
  ...
  private isGreenForDirection(light: TrafficLight, direction: Direction): boolean {
    const axis = direction === "left" || direction === "right" ? "horizontal" : "vertical";
    if (axis === "vertical") {
  ...
  private isYellowPhase(light: TrafficLight): boolean {
    return light.phase === "vertical" && light.timeInPhase >= Math.max(0, light.cycleSeconds - light.yellowSeconds);
  }

  private getStopLinePoint(light: TrafficLight, direction: Direction): Coordinate {
    const offset = 54;
    if (direction === "right") {
  ...
  private mustYieldToPedestrian(position: Coordinate, direction: Direction): boolean {
  ...
  private isPedestrianInsideRoadPart(pedestrian: Pedestrian, crosswalk: Crosswalk): boolean {
    const margin = 10;
  ...
  private forwardDistance(from: Coordinate, target: Coordinate, direction: Direction): number {
    if (direction === "right") {
  ...
  private isPointInsideRect(point: Coordinate, rect: { x: number; y: number; width: number; height: number }, margin = 0): boolean {
  ...
  private updatePedestrians(deltaSeconds: number): void {
    this.pedestrians = this.pedestrians.map((pedestrian, index) => {
      ...
      const distanceToTarget = Math.hypot(dx, dy);
      if (distanceToTarget <= 2) {
        const nextDirection: 1 | -1 = pedestrian.direction === 1 ? -1 : 1;
        return {
          ...pedestrian,
          direction: nextDirection,
          position: { ...target },
  ...
  private checkPedestrianCollision(): void {
  ...
  private finishGame(customMessage?: string): void {
    if (this.isFinished) {
      return;
    }
  ...
  private canMoveTo(position: Coordinate): boolean {
  ...
  private isPointOnRoad(point: Coordinate): boolean {
    return this.roads.some((road) => this.isPointInRectRoad(point, road));
  }

  private isPointInRectRoad(point: Coordinate, road: Road): boolean {
    const x = road.x ?? 0;
    const y = road.y ?? 0;
  ...
  private getSelectedOrder(): Order | null {
  ...
  private getCarriedOrder(): Order | null {
    if (!this.courier.carriedOrderId) {
      return null;
    }
  ...
  private prepareOrders(offsetSeconds: number): Order[] {
  ...
  private assignRandomArcadeOrder(): Order | null {
    if (this.currentMode !== "arcade" || this.courier.carriedOrderId) {
      return null;
    }
  ...
  private keyToDirection(key: string, code?: string): Direction | null {
  ...
  private drawRectRoadBase(road: Road): void {
    const x = road.x ?? 0;
    const y = road.y ?? 0;
    const width = road.width ?? 0;
    const height = road.height ?? 0;
  ...
  private drawRectRoadMarkings(road: Road): void {
    const x = road.x ?? 0;
    const y = road.y ?? 0;
    const width = road.width ?? 0;
    const height = road.height ?? 0;
  ...
  private drawHorizontalRoadMarkings(x: number, y: number, width: number, height: number, isMajor: boolean): void {
    const ctx = this.ctx;
  ...
  private drawVerticalRoadMarkings(x: number, y: number, width: number, height: number, isMajor: boolean): void {
    const ctx = this.ctx;
    ctx.strokeStyle = "rgba(255, 255, 255, 0.33)";
    ctx.lineWidth = 2;
    ctx.setLineDash([]);
  ...
  private drawBuilding(building: Building, color: string): void {
    const ctx = this.ctx;
    const depth = 16;
  ...
  private drawCenterTrafficSignal(light: TrafficLight): void {
    const ctx = this.ctx;
  ...
  private drawPedestrian(pedestrian: Pedestrian): void {
    const ctx = this.ctx;
    ctx.save();
  ...
  private drawCar(position: Coordinate, direction: Direction, color: string, label: string): void {
    const ctx = this.ctx;
  ...
  private drawCameraFlashOverlay(): void {
  ...
  private getActiveTarget(): Coordinate | null {
  ...
  private createCrosswalks(): Crosswalk[] {
    return [
  ...
  private createBuildings(): Building[] {
    return [
  ...
  private createSpeedCameras(): SpeedCamera[] {
    return [
  ...
  private createTrafficLights(): TrafficLight[] {
    return [
  ...
  private createTrafficCars(): TrafficCar[] {
    return [
  ...
  private createPedestrians(): Pedestrian[] {
    return [
  ...
  private getCurrentSpeedKmh(): number {
  ...
  private applyFriction(value: number, amount: number): number {
  ...
  private vectorToDirection(vector: Coordinate): Direction {
  ...
  private directionSignX(direction: Direction): number {
    if (direction === "right") {
      return 1;
    }
    if (direction === "left") {
  ...
  private directionSignY(direction: Direction): number {
    if (direction === "down") {
      return 1;
    }
    if (direction === "up") {
  ...
  private distance(left: Coordinate, right: Coordinate): number {
